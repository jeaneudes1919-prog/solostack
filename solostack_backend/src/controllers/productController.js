...
const streamifier = require('streamifier'); // Ne pas oublier d'installer : npm install streamifier

// --- FONCTION UTILITAIRE : Upload via Buffer (Indispensable pour Render) ---
...
exports.createProduct = async (req, res) => {
  // On utilise db.pool.connect() si ton export db.js contient { pool }
  const client = await db.pool.connect();
  
  try {
    await client.query('BEGIN');

    if (!req.file) throw new Error("L'image du produit est obligatoire.");

    // ✅ CORRECTION : Upload via Stream/Buffer
...
// --- 3. TOUS LES AUTRES MÉTHODES (Sans changement requis) ---
...
exports.deleteProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const owner_id = req.user.id;
    const checkQuery = `SELECT p.id FROM products p JOIN stores s ON p.store_id = s.id WHERE p.id = $1 AND s.owner_id = $2`;
...
exports.addProductReview = async (req, res) => {
  ...
  try {
    ...
    const purchaseCheck = await client.query(`
      ...
      WHERE pv.product_id = $1 AND o.user_id = $2 AND o.payment_status = 'paid' LIMIT 1
    `, [id, user_id]);
    if (purchaseCheck.rows.length === 0) return res.status(403).json({ error: "Achat requis pour noter." });
    await client.query(`INSERT INTO product_reviews (product_id, user_id, rating, comment) VALUES ($1, $2, $3, $4)`, [id, user_id, rating, comment]);
...
exports.getProductReviews = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await db.query(`SELECT pr.*, u.first_name, u.last_name FROM product_reviews pr JOIN users u ON pr.user_id = u.id WHERE pr.product_id = $1 ORDER BY pr.created_at DESC`, [id]);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
